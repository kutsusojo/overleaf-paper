...
%%%
%%% Please use "pasj02_template.tex" for preparing your manuscript.  
%%%
%%% ver.9 (2024/07/01) 
...
\title{ Usage of \texttt{pasj02.cls}}

\author{PASJ Editorial Office}% \orcid{0000-0000-0000-0000}}%

\altaffiltext{}{Astronomical Society of Japan, c/o National Astoronomical Observatory of Japan, 
 2-21-1 Osawa, Mitaka, Tokyo 181-8588, Japan}
\email{***@***.***.***}

\KeyWords{key word --- key word --- \dots --- key word }

\maketitle

\begin{abstract}
In this document (\texttt{pasj02\_usage.tex}), we provide a brief explanation about \texttt{pasj02.cls}, 
 the current version of PASJ's document class for authors. 
The class file, \texttt{pasj02.cls}, is prepared so that authors can typeset/preview 
 articles for PASJ under the \textit{standard} {\LaTeX} system.
...
\LaTeX{} style; that is, this manual shows only the differences of 
 functions provided by \texttt{pasj02.cls} and those in the \textit{standard} \LaTeX{}.  
Please also see ``Instructions to Authors'' available on 
 $\langle$\verb|https://academic.oup.com/pasj/pages/General_Instruction|$\rangle$. 
The author's guide in Japanese can be found at  $\langle$\verb|https://www.asj.or.jp/pasj/guide/|$\rangle$. 
\end{abstract}

%\pagewiselinenumbers

\section{Overview}
When \texttt{pasj02.cls} is applied to an article for PASJ, 
 the article should be prepared in the standard \LaTeX{} style with 
...
\bibitem[label(year)]{key} reference entry
...
\end{thebibliography}
\end{document}
\end{verbatim}
...
Please note that the class file \texttt{pasj02.cls} uses different typefaces from those in the current journal.  
That is, authors cannot obtain an identical image with the published article unless the class file is not changed or replaced. 



\section{Class options}

The class file \texttt{pasj02.cls} admits the following options:
\begin{itemize}
...
\item \texttt{twocolumn}: use two-column format for a submission 
\item \texttt{onecolumn}: use one-column format for a submission 
\item \texttt{proof}: typeset in draft-style 
\item \texttt{useamsfonts}: enable to use symbols defined in \texttt{amssymb.sty}
...
\noindent \underline{Single affiliation} \\
\noindent \verb/ \author{/%
    {John} \verb/\textsc{/{Smith}\verb/}/ \\ 
 \verb/  / and 
    {Paul} \verb/\textsc{/{Wood}\verb/}\orcid{/0000-0000-0000-0000\verb/}}/\\
 \verb/ \affil{/\textsl{Affiliation}\verb/}/\\ 
 \verb/   \email{/\textsl{Address${}_{1}$}, \textsl{Address${}_{2}$}\verb/}/

\noindent \underline{Two or more affiliations} \\
 \verb/   \author{/%
    {John} \verb/\textsc{/{Smith}\verb/}/%\\
 \verb/\altaffilmark{1}/\\
 \verb/\altemailmark/ \\
 ...
 \verb/\altaffilmark{2} }/\\
 \verb/\altemailmark\orcid{0000-0000-0000-0000} / \\
...
in its first argument and  \verb/\altemailmark/ will give the label corresponding e-mail. 
...
For cross-references of sections, figures, equations etc., the pair of 
 commands, \verb/\label/ and \verb/\ref/, is available. 
Since the usage of these two commands is exactly the same as that in the standard \LaTeX, 
 we leave the explanation about \verb/\label/ and \verb/\ref/ to adequate instructions of \LaTeX.

For in-text citations, \texttt{pasj02.cls} provides the system of \verb/\cite/ and 
...
by \texttt{pasj02.cls} is as follows:
\begin{quote}
...
\end{quote}
Note that the input of \textsl{label}, in the form of ``\textsl{author(year)},''   
...
In addition to the usual \verb/\cite/ command, \texttt{pasj02.cls} provides 
...
If a comma-separated list of reference keys is given as an argument 
 ...
 of reference labels. 
For other commands, readers can easily find the result for a list 
 of keys by simple experiments. 


\section{Mathematical formulas}

For mathematical formulas,  \texttt{pasj02.cls} allows 
 \verb/$...$/ and ``math'' environment for in-text formulas and  
 ``equation'' and ``eqnarray'' environments for displayed formulas.   

For mathematical symbols, \texttt{pasj02.cls} allows one to use symbols 
 provided by the standard \LaTeX{} and some more symbols given 
...
\caption{%
 ...
 This diagram is drawn with a ``picture'' environment, no EPS file is included.  
 {Alt text: Rectangle drawn with lines.  }
  }%
  \label{fig:usage}
\end{figure} 

To place figures appropriately, the usual ``figure'' environment is available. 
As in the standard \LaTeX, \texttt{pasj02.cls} allows the following description and
 figure~\ref{fig:usage} is an example of usage of the ``\mbox{figure}'' environment. 

The class file supports the embeddings of graphic files in the EPS and PDF format 
 as its default.  
Upon compilation, the class file loads the \texttt{graphicx} and 
 \texttt{color} packages. 
...
For the location of figure files (or the directory/folder in which 
 figure files exist), \texttt{pasj02.cls} assumes that figure files and 
...
Note that this ``longtable'' environment obtains the maximum size 
 of the width of cells in each column via the aux file. 
Therefore, it is required to typeset at least twice to produce a correct table.  
For the meanings of \verb/\endhead/ etc., see ``The \LaTeX{} Companion'' or 
 appropriate instruction for \LaTeX.
...
Though the \texttt{longtable} package provides some parameters, like 
 \verb/\LTleft/ and \verb/\LTpre/, the \texttt{pasj02.cls} class file 
...
The class file \texttt{pasj02.cls} provides the \verb/\draft/ command 
...
The symbols in table~\ref{table:extramath} are provided by \texttt{pasj02.cls}. 
Some of them are also defined in \texttt{amssymb.sty}, and 
 the definitions of such commands are replaced with those 
 in \texttt{amssymb.sty} if \texttt{useamsfont} option is specified. 
...
\hbox to6pt{\footnotemark[$*$]\hss}\unskip% 
 ...
 If the \texttt{amssymb} package is available, then the \texttt{useamsfonts} class option enables 
  to use the symbols defined by \texttt{amssymb} package. 
...
The class file \texttt{pasj02.cls} provides the commands for astronomical symbols:  
 for the symbol of the sun, \verb/\Sol/ and \verb/\solar/ produce the symbol \Sol. 


\subsection{Description of time/angle, atoms etc.}

To produce the description of time/angle like ``\timeform{1h23m45.67s}''
 or ``\timeform{6D54'32.1''}'', the class file \texttt{pasj02.cls} provides 
...
Though the file \texttt{pasj02.cls} also provides (\texttt{aastex}-like) 
...
\item[{\normalfont\texttt{\textbackslash iaucirc}:}] \iaucirc\\
   IAU Cirulars 
\item[{\normalfont\texttt{\textbackslash icarus}:}] \icarus\\
   ICARUS  
...
Though class file \texttt{pasj02.cls} does not inihibit the use 
 of \verb/\def/, \verb/\newcommand/ etc., it is \textit{not} recommended 
 to define a user's own command. 
Note that a user's own trivial abbriviations might cause fatal errors by 
 changing the existing commands or by interfering with macros defined in 
 other articles (or in the class file used for publication). 
Every author should remember that \textit{no} journal 
